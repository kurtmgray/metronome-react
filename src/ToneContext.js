...
const reducer = (state, action) => {
    switch (action.type) {
        ...
        case "secondToLastClick":
            return {...state, secondToLastClick: action.value}  
        case "lastClick":
            return {...state, lastClick: action.value}
        case "measSound":
        ...
        case "syncActiveProgramTitle":
            console.log(state.programs.find((program) => program.id === action.value))
            return {
                ...state,
                activeProgram: state.programs.find((program) => 
                    program.id === action.value
                )
            }     
        default:
            return state
...
export const ToneContext = ({children}) => {
    ...
    useEffect(() => {
        ...
        if (state.isPlaying && !state.programMode && measLoop.current.state === 'stopped') {
    ...
    useEffect(() => {
        if (state.activeProgram && state.programMode && state.isPlaying) {
            ...
            const startMeasures = state.activeProgram.presets.map(({iterations}, i) => {
            ...
            state.activeProgram.presets.map((preset, i) => {
                ...
                return (
                    Object.keys(preset.sounds).forEach(sound => {
                        let loop = new Tone.Loop(time => {
                            players.player(sound).start(time + ".2")
                        }, length(sound)[0])
                        loop.iterations = sound === 'meas' ? 
                            preset.iterations : 
    ...
    const methods = {
        ...
        handleTap: () => {
            ...
            console.log(tempo) 
            dispatch({ type: 'secondToLastClick', value: state.lastClick })
            dispatch({ type: 'lastClick', value: timeNow })
            if (tempo > 250) tempo = 250
            if (tempo < 30) tempo = 30
            if (tempo) {
              dispatch({type: 'tempo', value: tempo}) 
            }
        },
        handleToggle: () => {
        ...
        handleSavePreset: (e) => {
            let programCopy = state.programs.find(program => program.id === e.target.id)
            const preset = {
                id: uuidv4(), // uuid
            ...
            if (!emptySound){
                ...
                dispatch({ type: 'addPreset', id: programCopy.id, value: programCopy.presets })
        ...
        handleSelectProgram: (e) => {
            const activeProgram = state.programs.find(program => program.id === e.target.id)
